programa {
  ...
  funcao inicio() {
    ...
    inteiro valor = 0, l, c, tabela[QTD][7], classificacao[QTD][8] // Classifcação	Pontos	Jogos	Vitórias	Empates	Derrotas	Gols Pro	Gols Contra	Saldo
    inteiro tam=0
    ...
    para (inteiro i = 0; i < 4 - 1; i++) {
      para (inteiro j = 0; j < 4 - i - 1; j++) {
        ...
        se(classificacao[j][0] < classificacao[j+1][0] ou  // Pontos
           (classificacao[j][0] == classificacao[j+1][0] e classificacao[j][2] < classificacao[j+1][2]) ou  // Vitórias
